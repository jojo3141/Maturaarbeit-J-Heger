...
with open("Ergebnisse/LBL_solutions.txt", "r") as file:
    all_LBL_solutions = [eval(line.strip()) for line in file]

with open("Ergebnisse/CFOP_solutions.txt", "r") as file:
    all_CFOP_solutions = [eval(line.strip()) for line in file]

with open("Ergebnisse/CFCE_solutions.txt", "r") as file:
    all_CFCE_solutions = [eval(line.strip()) for line in file]

with open("Ergebnisse/ROUX_solutions.txt", "r") as file:
...
def create_diagram(data1, data2, data3, data4, xlabel="", ylabel="", title="",
                   save_in_place="Ergebnisse/I_forgot_to_save_it", average=False):
...
def create_move_count_diagram():
    ...
    create_diagram(all_LBL_move_counts, all_CFOP_move_counts, all_CFCE_move_counts, all_ROUX_move_counts, "Zuganzahl pro Lösung", "Häufigkeit", "Wahrscheinlichkeit der Zuganzahl pro Lösung", "Ergebnisse/move_count_histogramm2.png", average=True)
...
def create_time_diagram():
    ...
    create_diagram(all_LBL_times, all_CFOP_times, all_CFCE_times, all_ROUX_times, "Lösungszeit in Sekunden", "Relative Häufigkeitsdichte", "Verteilung der Lösungszeit", "Ergebnisse/time_histogram10.png")
