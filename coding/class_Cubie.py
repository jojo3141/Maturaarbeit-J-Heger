...
class Alg:
    ...
    def rotate_left(self):
        new_alg = list(self.alg)  # create a copy of self.alg

        for i in range(len(new_alg)):
            if new_alg[i] == "R":
                new_alg[i] = "F"
            elif new_alg[i] == "Ri":
                new_alg[i] = "Fi"
        ...
        return new_alg  # return the copied version of self.alg with the changes made
    ...
    """def alg_list(self):
        alg_list = []
        alg_list.append(self.alg)
        alg_list.append(self.rotate_left())
        self.alg = self.rotate_left()
        alg_list.append(self.rotate_left())
        self.alg = self.rotate_left()
        alg_list.append(self.rotate_left())
        return alg_list"""
...
#OLL Algorithms nach http://algdb.net/puzzle/333/oll
...
oll26 = ["Ri", "Ui", "R", "Ui", "Ri", "U2", "R"] #sune
oll27 = ["R", "U", "Ri", "U", "R", "U2", "Ri"] #sune
...
cmll10 = ["F", "R", "U", "Ri", "Ui", "R", "U", "Ri", "Ui", "R", "U", "Ri", "Ui", "Fi"] #0y
...
cmll16 = ["U", "R", "U2", "R2", "F", "R", "Fi", "R", "U2", "Ri"]# 2y

cmll17 = ["Ri", "U", "Li", "U2", "R", "Ui", "Ri", "U2", "R", "L"]
cmll18 = ["F", "R", "Ui", "Ri", "Ui", "R", "U", "Ri", "Fi", "R", "U", "Ri", "Ui", "Ri", "F", "R", "Fi"] # 4 on top
...
cmll24 = ["Li", "Ui", "L", "Ui", "Li", "U", "L", "F", "R", "U", "Ri", "Fi"] #no yellow on top
...
cmll30 = ["U2", "R", "U", "Ri", "U", "Ri", "F", "R", "Fi", "R", "U2", "Ri"] # 1 yellow on top
...
cmll42 = ["Ui", "F", "R", "U", "Ri", "Ui", "Fi"] # 2 yellow on top

cmll_group1 = [cmll1, cmll2, cmll3, cmll4, cmll5, cmll6, cmll25, cmll26, cmll27, cmll28, cmll29, cmll30] # 1 yellow on top
cmll_group2 = [cmll7, cmll8, cmll9, cmll10, cmll19, cmll20, cmll21, cmll22, cmll23, cmll24, cmll17, cmll18] # no or 4 yellow on top
cmll_group3 = [cmll11, cmll12, cmll13, cmll14, cmll15, cmll16, cmll31, cmll32, cmll33, cmll34, cmll35,
               cmll36, cmll37, cmll38, cmll39, cmll40, cmll41, cmll42] # 2 yellow on top
...
ell_group = [ell1, ell2, ell3, ell4, ell5, ell6, ell7, ell8, ell9, ell10, ell11, ell12, ell13, ell14,
             ell15, ell16, ell17, ell18, ell19, ell20, ell21, ell22, ell23, ell24, ell25]
...
def spell_alg(alg, short_version=False):
    ...
    if short_version:
        ...
        formatted_str = formatted_str.replace("y\033[0m\033[32my\033[0m\033[32my", "y'")  # replace "yyy" with "y'"
        ...
        #formatted_str = re.sub(r'(?<!\w)([a-z])', r' \1', formatted_str)
        #formatted_str = re.sub(r"([2'RFLBDU])([yx])", r"\1 \2", formatted_str)
        ...
        #return spelled alg, move count, rotation count (-2 cause it starts with xx)
...
def count_moves(lst):
    count = 0
    for elem in lst:
        if any(char in elem for char in ['R', 'L', 'D', 'U', 'B', 'F']):
            count += 1
        if '2' in elem:
            count += 1
    print(f"Moves: {count}, Rotations: {count}")
